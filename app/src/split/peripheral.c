...
static int select_first_available_transport(void) {
    ...
    // it. A single radio is time-multiplexed between BLE and ESB, so abandoning
    // a working link to chase a higher-priority transport that merely became
    // "available" again (e.g. the BLE retry timer firing while ESB is connected
    // in dongle mode) would drop the halves every retry interval. We only
    // re-select once the active transport reports DISCONNECTED.
